...
function isEligibleForPromotion(candidate) {
  ...
  return (
    trialStatus === STATUSES.TRIAL_APPROVED &&
    qualityScore >= 80 &&
    reputationScore >= 60 &&
    promotionStatus !== STATUSES.PROMOTED
  );
}

function pickBestPromotionCandidates(candidates, limit) {
  return candidates
    .filter(isEligibleForPromotion)
    .sort((a, b) => {
...
async function runPromotion({ country, category, limit = 10 }) {
  ...
  const candidates = pickBestPromotionCandidates(allCandidates, normalizedLimit);
  ...
  for (const candidate of candidates) {
    const aiSourceId =
      ...
      `${normalizedCountry}_${normalizedCategory}_${candidate.domain || buildRunId("aisource")}`;
    ...
    batch.set(
      aiSourceRef,
      {
        ...
        source_type: candidate.source_type || "discovery-promoted",
